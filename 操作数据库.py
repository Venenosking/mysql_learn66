...
p_database = {key + ' ('+file_name+')': value for key, value in {
   ...
   "修改数据库名称 3种方式":"1.先把表导出来,再导入新的数据库 2.先创建一个新数据库,然后将源数据库下的表重命名到新的数据库下 rename table 新数据库.表名 to 旧数据库.表名 3.create table 新数据库名称.源数据库表 like 源数据库名称.源数据库表; 但是有弊端,只有表结构没有表数据",
