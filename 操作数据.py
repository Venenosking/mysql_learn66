...
def p_datas():
    p_data = {key + ' (' + file_name + ')': value for key, value in {
       ...
       "删除表中指定数据between and": "delete from 表名 where 字段名 between 具体数值1 and 具体数值2 ;",
       "删除表中指定数据> >= < <=": "delete from 表名 where 字段名 这里写>=或者> 具体数值1 and 字段名 这里写<=或者< 具体数值2 ;",
       "删除表中指定数据in,not in": "delete from 表名 where 字段名 in (具体值1 , 具体值2);",
       "删除表中指定数据like": "delete from 表名 where 字段名 like '具体的值'; ",
       "删除表中指定数据regexp正则表达式": "delete from 表名 where 字段名 regexp '具体的值';",
       "更新全部数据": "update 表名/视图名 set 字段名=具体的值;",
       "更新指定数据=": "update 表名/视图名 set 字段名=具体的值 where 字段名 = 具体的值 ; ",
       "更新指定数据between and": "update 表名/视图名 set 字段名=具体的值 where 字段名 between 具体的值1 and 具体的值2 ;",
       "更新指定数据> >= < <=": "update 表名/视图名 set 字段名=具体的值 where 字段名 这里写>=或者> 具体的值1 and 字段名 这里写<=或者< 具体的值2 ; ",
       "更新指定数据in/not in": "update 表名/视图名 set 字段名=具体的值 where 字段名 这里写in或者not in (具体的值);",
       "更新指定数据like": "update 表名/视图名 set 字段名=具体的值 where 字段名 like '具体的值' ; ",
       "更新指定数据regexp": "update 表名/视图名 set 字段名=具体的值 where 字段名 regexp '具体的值' ; ",
...
if __name__ == "__main__":
    ...
    for key, value in a_part.items():
        ...
        key = '((⏱️=1500))' + key
        print('\n', key, value)
    print('\n一共有 ' + str(len(a_part)) + ' 个'+' '+str(len(p_datas())))
