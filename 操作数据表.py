...
def p_tables():
    p_table = {key + ' (' + file_name + ')': value for key, value in {
        ...
        "删除unique约束": "先用show create table 表名;查看unique名 然后 alter table 表名 drop index unique名;",
        "删除default默认值": "alter table 表名 alter 字段名 drop default;",
        "删除foreign key外键": "alter table 表名 drop foreign key 外键名;",
        "创建表 3种方式": "1.create table 表名();2.create table 新表名 like 旧表名;3.create table 表名 select 语句;",
        "查看所有表": "show tables;",
        "查看表结构": "desc 表名;",
        ...
        "添加新字段": "alter table 表名称 add 新字段名称 字段类型 not null unique ... first(after 字段2);",
        "修改字段名和字段类型(字段位置,自增)": "alter table 表名称 change 旧的字段名 新的字段名 新的数据类型; alter table 表名称 modify 字段名 新的数据类型;",
        "添加check约束": "alter table 表名称 add constraint 约束名称 check (约束条件);",
        "添加unique": "alter table 表名称 add constraint 唯一约束名称 unique(字段名称);",
        "添加默认值": "alter table 表名称 alter 字段名 set default 默认值;",
        "添加外键": "alter table 表名 add constraint 外键名称 foreign key (外键字段) references 父表名称(父表主键名称);",
